...
public class DishController {
    ...
    public R<Page> page(int page,int pageSize,String name){
        ...
        List<Dish> pageInfo = dishService.page(page,pageSize,name);
        DishDto dishDto = new DishDto();

        //对象拷贝
        BeanUtils.copyProperties(pageInfo,dishDto,"records");

        List<Dish> records = dishService.getRecords();
    ...
    public R<List<DishDto>> list(Dish dish){
        ...
        status = 1;
        List<Dish> list = dishService.list(categoryId,status);
        ...
        dishDtoList = list.stream().map((item->{
            ...
            List<DishFlavor> dishFlavorList = dishFlavorService.list(dishId);
